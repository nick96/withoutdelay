# Based on: https://github.com/nltk/nltk_contrib/blob/master/nltk_contrib/timex.py
...
ISO = "\d+[/-]\d+[/-]\d+ \d+:\d+:\d+\.\d+"
YEAR = "((?<=\s)\d{4}|^\d{4})"
REGEXP1 = "((\d+|(" + NUMBERS + "[-\s]?)+) " + dmy + "s? " + EXP1 + ")"
REGEXP2 = "(" + EXP2 + " (" + dmy + "|" + WEEK_DAY + "|" + MONTH + "))"
...
def ground(tagged_text, base_date):
    ...
    for timex in timex_found:
        ...
        if re.search(NUMBERS, timex, re.IGNORECASE):
            split_timex = re.split(
                r"\s(?=days?|months?|years?|weeks?)", timex, re.IGNORECASE
            )
            value = split_timex[0]
            unit = split_timex[1]
            num_list = map(
                lambda s: hashnum(s), re.findall(NUMBERS + "+", value, re.IGNORECASE)
            )
            timex = f"{sum(num_list)} {unit}"
        ...
        tagged_text = re.sub(
            "<TIMEX2>" + timex_ori + "</TIMEX2>",
            '<TIMEX2 val="' + timex_val + '">' + timex_ori + "</TIMEX2>",
            tagged_text,
        )
